...
*** Core SDLC(Systems development life cycle) and DevOps Principles
...
***** Nullptr dereference
***** ASLR{Address space layout randomization} and DEP{Data Execution Prevention} is common defense against buffer overflows
...
* the credentials are passed in the
